...
# ./Air_Corridor_Design/Optimization/path_planning_astar.py
"""
Module: path_planning_astar
This module contains the implementation of the A* algorithm for 3D path planning in emergency landing scenarios.

Functions:
    a_star_3d(field, START, END, index): A* algorithm for 3D path planning.
"""
...
def a_star_3d(field, START, END, index):
    '''
    ...
    for i in range(field.shape[0]):
        for j in range(field.shape[1]):
            for k in range(field.shape[2]):
                ...
                if index == 0:
                    array = np.array([float('inf'), 0.0, 0.0, 0.0, 0.0])
                elif index == 1:
                    array = np.array([float('inf'), float('inf'), float('inf'), 0.0, 0.0])
                else:
                    array = np.array([0.0, float('inf'), 0.0, 0.0, 0.0])
    ...
    state_vector = [Cost_all_i, Si, Cumulative Rnoise, Cost_rescue, j=0->i(1-pj)]"""
    init_distance = np.array([0.0, 0.0, start.noise, start.Cr_r, 1.0])
    start.set_statevector(init_distance)
    
    # Heuristic function (straight-line distance to the goal)
    # Revise!!!
    def heuristic(node, end):
        return ER_Weight_distance*((node.x - end.x) ** 2 + (node.y - end.y) ** 2 + (node.z - end.z) ** 2) ** 0.5
    ...
    while not q.empty():
        ...
        for neighbor in get_neighbors(current_p, field):
            ...
            if sv_lt(distance,neighbor.StateVector,index):
                ...
                neighbor_g_cost = ER_Weight_distance * neighbor.StateVector[0] + \
                    Enlarge_param_tpr * (1 - ER_Weight_distance - ER_Weight_rescue) * neighbor.StateVector[1] + \
                    Enlarge_param_rescue * ER_Weight_rescue * neighbor.StateVector[3]
                neighbor_h_cost = heuristic(neighbor, end)
                neighbor_f_cost = neighbor_g_cost + neighbor_h_cost
                neighbor.father = current_p
                q.put((neighbor_f_cost, neighbor))
    ...
    if end.father==None:
        ...
        result = (
            ...
            float(ER_Weight_distance * field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[0] + \
                    Enlarge_param_tpr * (1 - ER_Weight_distance - ER_Weight_rescue) * field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[1] + \
                    Enlarge_param_rescue * ER_Weight_rescue * field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[3]),\
            float(Enlarge_param_tpr * (1 - ER_Weight_distance - ER_Weight_rescue) * field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[1]/ \
                  (ER_Weight_distance * field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[0]+0.00001)),\
            float(Enlarge_param_rescue * ER_Weight_rescue * field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[3] / \
                  (ER_Weight_distance * field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[0]+0.00001)), \
            float(field[path[-1][0]][path[-1][1]][path[-1][2]].StateVector[4])
        )
        
        end_time = time.time()

        Objective = round(result[4], 3)
        NumUnit = field.shape[0] * field.shape[1] * field.shape[2]
        time_all = round(end_time-start_time, 3)
        print(f"A*  : [Distance] {round(result[1], 2):<6}  [TPR] {round(result[2], 5):<6} [Rescue] {round(result[3], 4):<6}  "
              f"[Objective] {Objective:<10}   [Time] {time_all:<6} s  "
              f"[Number of units] {NumUnit:<10} [Ratio (TPR/Distance)] {round(result[5], 3):<6}  "
              f"[Ratio (Rescue/Distance)] {round(result[6], 3):<6}")
        return path, result