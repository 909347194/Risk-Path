...
由此得到的 $t_j$ 即为在线时间采样的查询时刻。算法据此从第~\ref{sec:tpr_models}~节建立的动态风险张量中提取当前位置的坠机危险率 $\lambda_{\mathrm{cr}}^{(j)}$、致死后果 $e_f^{(j)}$、财产后果 $e_p^{(j)}$ 与噪声强度 $r_n^{(j)}$：
