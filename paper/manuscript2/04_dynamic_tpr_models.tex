...
综上，本节建立了坠机概率 $P_{\mathrm{crash}}(x,y,z,t)$、致死后果 $E_{\mathrm{fatality}}(x,y,z,t)$、财产后果 $E_{\mathrm{property}}(x,y,z)$ 和噪声成本 $R_{\mathrm{noise}}(x,y,z,t)$ 四项动态风险分量的物理计算模型。结合第~\ref{sec:system_architecture}~节定义的多目标优化框架与约束条件，上述风险分量将作为第~\ref{sec:algorithm}~节路径规划算法的在线查询输入。
